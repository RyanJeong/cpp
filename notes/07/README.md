...
* **기반 클래스 형 포인터는 기반 클래스를 상속한 모든 클래스 형 객체를 가리킬 수 있음**
  ...
  * 기반 클래스 형이 파생 클래스 형 객체를 가리키면 **기반 클래스 영역**만을 사용할 수 있음
...
* 컴파일 시점에 어떤 함수 호출이 발생하였을 때 이와 연관되는 함수 정의가 명확한 경우
...
* 런타임 시점에 함수 호출과 함수 정의 간 연관을 확정
  ...
  * 지연 바인딩 (late binding)
* 컴파일 시점에 어떤 함수 호출이 발생하였을 때 이와 연관되는 함수가 **모호한 경우**
  * 런타임 시점에만 함수를 호출하는 객체의 실제 형이 확정되는 경우
...
* 동적 바인딩이 동작하기 위한 필수 조건은 **가상 함수와 기반 클래스 형 포인터 또는 참조**
