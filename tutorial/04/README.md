...
#### 객체(<i>object</i>)
* 프로그램으로 구현하고자 하는 <b>실세계에 존재하는 대상</b>
...
* 특정 대상으로부터 추상화한 결과물(속성들과 행위들) 중에서 관련있는 것끼리 묶어놓은 것을 캡슐화(<i>encapsulation</i>)라고 함
    * 예를 들어, 자동차를 추상화했다고 가정하면, 여러 속성들과 행위들을 도출할 수 있음
        * 속성에는 자동차의 속도, 주유량, 주행거리, ... 등이 있음
        * 행위에는 감속, 가속, 조향, ... 등이 있음
    * 이때, 가속 행위와 관련있는 속성들을 묶는 것을 캡슐화라고 할 수 있음 
* 캡슐화한 것들을 하나로 묶는 추상적인 주체를 클래스(`class`)라고 함
    * 캡슐화한 것들은 결국 하나의 공통된 객체를 토대로 탄생한 것들이며, 이를 묶을 수 있는 큰 범주가 클래스임
    * <b>객체로부터 추상화를 거친 결과가 클래스이며, 이 클래스는 여러 개의 캡슐화된 것들을 포함할 수 있음</b>
...
#### 객체, 클래스, 인스턴스
* 객체(<i>object</i>)는 <b>프로그램으로 구현할 실세계에 존재하는 대상</b>이고, 클래스(<i>class</i>)는 <b>객체를 프로그램으로 실체화하기 위한 설계도</b>이며, 인스턴스(<i>instance</i>)는 <b>클래스를 기반으로 메모리에 할당된 실체</b>다. 
