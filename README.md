...
* 변수 선언과 동시에 초기화를 하는 것이 가장 좋지만, 초기치가 정해지지 않아 변수 선언과 초기화를 동시에 할 수 없는 경우라면 변수 선언 부분과 해당 변수를 초기화하는 부분이 코드 상에서 최대한 근접해야 한다. 이 거리가 멀어질수록 변수를 잘못 사용함에 따른 오류 발생 가능성이 그만큼 높아질 것이다. 
