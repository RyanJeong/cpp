...
* There is an argument for *always* using a second line:
...
* `v`는 표준 라이브러리 컨테이너 중 하나다(`vector` or `string`). 일반적인 PC에서 오류가 발생할 수 있는 경우는 벡터의 크기가 $2^{31}$ 을 넘어가는 경우 뿐이다. 이 경우는 거의 등장하지 않는다.
* 이 부분에 대한 자세한 설명 또는 $2^{31}$ 크기보다 더 큰 벡터가 필요한 경우에는 §25.5.3을 참고한다.

## The Real Stroustrup Interview

* [https://www.stroustrup.com/ieee_interview.pdf](https://www.stroustrup.com/ieee_interview.pdf)
